  ...
  <div class="justify-center items-center p-[16px] md:px-[100px]">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-[24px]">
      <div
        ...
        :key="index"
        class="flex flex-col rounded-[40px] items-center bg-white p-[24px] h-full"
      >
        <h2 class="font-Golos font-[600] text-[16px]">{{ plan.title }}</h2>
        ...
        <div :class="{ 'mt-4': index !== plans.length - 1, 'mt-[39px]': index === plans.length - 1 }" class="flex flex-col items-center w-full">
          <button
            :class="{
              ...
              'w-full': true,
              'shadow-customOrangeShadow': index !== plans.length - 1,
            }"
            @click="scrollTo('section1')"  class="py-[16px] px-[80px] rounded-[46px] font-[500] text-[13px] text-white mb-4">
            Tarifni sotib oling
          </button>
          <button
            @click="toggleContent(index)"
            class="py-[16px] px-[80px] rounded-[46px] w-full md:hidden block font-[500] text-[13px] border border-solid">
            Ko’proq ko’rish
          </button>
        </div>
